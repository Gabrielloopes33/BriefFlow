...
export function configToHtml(config: HtmlSlideConfig): string {
  ...
  const titleHtml = `<h1 style="margin:0;font-family:'${config.title.fontFamily}',sans-serif;font-size:${clamp(config.title.fontSize, 24, 120)}px;line-height:${titleLineHeight};font-weight:${config.title.fontWeight};color:${escapeHtml(config.title.color)};">${escapeHtml(config.title.text)}</h1>`;
  const subtitleHtml = `<p style="margin:0;font-family:'${config.subtitle.fontFamily}',sans-serif;font-size:${clamp(config.subtitle.fontSize, 14, 64)}px;line-height:${subtitleLineHeight};font-weight:${config.subtitle.fontWeight};color:${escapeHtml(config.subtitle.color)};">${escapeHtml(config.subtitle.text)}</p>`;
  const ctaHtml = config.ctaButton.visible ? `<div style="margin-top:${ctaTopMargin}px;"><button style="display:inline-flex;padding:14px 26px;border:none;border-radius:${ctaRadius}px;background:${escapeHtml(config.ctaButton.backgroundColor)};color:${escapeHtml(config.ctaButton.textColor)};font-family:'Inter',sans-serif;font-size:24px;font-weight:700;line-height:1;">${escapeHtml(config.ctaButton.text)}</button></div>` : '';

  let contentHtml = `${accentHtml}${titleHtml}${subtitleHtml}${ctaHtml}`;

  if (templateVariant === 'glass-card') {
    contentHtml = `<div style="display:flex;flex-direction:column;gap:18px;"><div style="display:flex;justify-content:space-between;align-items:flex-start;gap:18px;flex-wrap:wrap;">${accentHtml}<div style="max-width:72%;font-family:'Inter',sans-serif;font-size:18px;line-height:1.45;font-weight:600;color:${escapeHtml(config.subtitle.color)};text-transform:uppercase;letter-spacing:0.08em;">${escapeHtml(config.subtitle.text)}</div></div>${titleHtml}${ctaHtml}</div>`;
  } else if (templateVariant === 'editorial-band') {
